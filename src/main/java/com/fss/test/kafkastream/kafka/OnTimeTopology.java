...
@Component
@Slf4j
public class OnTimeTopology {
    @Value(value = "${test.kafka.transLogInboundTopic}")
    private String transLogInboundTopic;
    @Value(value = "${test.kafka.ontimeCandleStickTopic}")
    private String ontimeCandleStickTopic;

    private static final Serde<String> STRING_SERDE = Serdes.String();
    
    @Resource(name="onTimeStreamsBuilder")
    StreamsBuilder  onTimeStreamsBuilder;

    @PostConstruct
    public void buildPipeline() throws InterruptedException {
        val storeSupplier = Stores.inMemoryKeyValueStore("in-mem");
        log.info("Init ontime stream");
        onTimeStreamsBuilder.stream(transLogInboundTopic, Consumed.with(STRING_SERDE, TranslogSerdes.serdes()).withTimestampExtractor(new TransLogTimeStampExtractor()))
        ...
        .toStream()
        .to(ontimeCandleStickTopic, Produced.with(getStringWindowedSerde(), CandleStickSerdes.serdes()));
        // candleStickKStream.peek((key,value) -> log.warn("Generate new stick; stick: {}", value));
        onTimeStreamsBuilder.build();
    }
